        ...
        <% loop $CalendarDays %>
            <div class="day_num<% if not $IsCurrentMonth %> ignore<% end_if %><% if $IsSelected %> selected<% end_if %> <% if not $IsCurrentMonth %>not-current-month<% end_if %>">
                ...
                <% loop $EventDays %>
                    ...
                    <dialog id="event-modal-{$Date}" class="event-modal" <% if $ID == $Top.CurrentEventDayID %>data-autoopen="true"<% end_if %>>
                        <button class="event-modal-close" onclick="document.getElementById('event-modal-{$Date}').close()">×</button>
                        <div class="modal-header">
                            <h2 class="event-title">$Parent.Title</h2>
                            <h3 class="event-title">$Title</h3>
                            <% if $Location %>
                                <p class="event-date">$RenderDateWithTime, $Location</p>
                            <% else %>
                                <p class="event-date">$RenderDateWithTime</p>
                            <% end_if %>
                        </div>
